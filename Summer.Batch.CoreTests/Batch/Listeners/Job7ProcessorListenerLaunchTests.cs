...
{
    /// <summary>
    /// Tests a simple flat file reader, processor, writer, with the processor used as a pre and postprocessor
    /// </summary>
    [TestClass()]
    public class Job7ProcessorListenerLaunchTests : AbstractListenersLaunchTests
    {
        private static readonly string TestPathIn = Path.Combine(TestDataDirectoryIn, "Job7In.txt");
        private static readonly string TestPathOut = Path.Combine(TestDataDirectoryOut, "Job7Out.txt");
        ...
        {
            RunJob("Job7.xml", "Job7", new MyUnityLoaderJob7());
            // Post controls
        ...
        /// </summary>
        private class MyUnityLoaderJob7 : UnityLoader
        {
            ...
            public override void LoadArtifacts(IUnityContainer unityContainer)
            {
                ...
                // Registering Processor which is also listener
                unityContainer.RegisterStepScope<IItemProcessor<Person, Person>, UppercaseProcessorListener>("processor");
